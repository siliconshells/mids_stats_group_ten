...
To assess multicollinearity, Variance Inflation Factors (VIFs) were calculated for each predictor (Appendix 1-C). All VIF values were below the threshold of 5, indicating that multicollinearity is not a major issue in the model. **Non-Hispanic White** had the highest VIF (3.46), suggesting a moderate level of correlation with other predictors, but it does not indicate a severe problem.
...
# Print AIC
print(aic)
deviance_value <- deviance(multinom_model)
...
kable(confusion_matrix, caption = "Confusion Matrix") %>%
  kable_styling(full_width = FALSE, position = "center", bootstrap_options = c("striped", "hover"), font_size = 8)
```
...
# Print AIC
print(aic_value)
# Calculate deviance of the fitted multinomial model
deviance_value <- deviance(multinom_model2)
...
**Appendix 2-1: Multiple Linear Regression Model Coefficients**
